# Especificação de Requisitos Funcionais (EARS) - RemindMe

Este documento contém os Requisitos Funcionais do sistema **RemindMe**, estruturados no padrão **EARS** (*Easy Approach to Requirements Syntax*).
...
* **RF12**: **IF** um usuário tentar converter um orçamento que não esteja aprovado, **THEN THE SYSTEM SHALL** impedir a conversão e informar o motivo.
...
* **RF16**: **IF** o desconto informado ultrapassar a alçada do usuário, **THEN THE SYSTEM SHALL** solicitar aprovação de um usuário autorizado.
...
* **RF24**: **IF** a data de vencimento de um título for ultrapassada sem pagamento registrado, **THEN THE SYSTEM SHALL** identificá-lo como vencido.
...
* **RF34**: **WHILE** um título permanecer vencido, **WHEN** atingir uma etapa de cobrança posterior ao vencimento, **THE SYSTEM SHALL** enviar a cobrança configurada.
...
* **RF38**: **IF** uma cobrança exigir intervenção manual, **THEN THE SYSTEM SHALL** encaminhar a tarefa ao responsável configurado.
...
* **RF46**: **WHERE** uma obrigação exigir evidência, **WHEN** o responsável concluir a obrigação, **THE SYSTEM SHALL** permitir anexar a evidência correspondente.
...
* **RF48**: **IF** uma obrigação ultrapassar seu prazo sem registro de cumprimento, **THEN THE SYSTEM SHALL** identificá-la como atrasada.
* **RF49**: **IF** uma obrigação permanecer atrasada pelo período configurado, **THEN THE SYSTEM SHALL** escalar o alerta ao proprietário ou responsável superior.
...
* **RF58**: **IF** um lançamento não possuir classificação válida, **THEN THE SYSTEM SHALL** identificá-lo como pendente de classificação.
